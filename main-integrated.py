...
async def list_streams():
    ...
    try:
        ...
        for stream_id, info in live_stream_handler.active_streams.items():
            ...
            # Kibővített információk visszaadása
            stream_info = {
                "id": stream_id,
                "url": info.source.url,
                ...
                "recording_path": info.recording_path,
                "transcription_id": info.transcription_id
            }
...
async def get_stream(stream_id: str):
    ...
    try:
        ...
        # Kiegészített információk visszaadása
        response_dict = {
            "id": stream_id,
            "url": info.source.url,
            "type": info.source.type,
...
async def stop_stream_recording(stream_id: str):
    ...
    try:
        ...
        # Felvétel elérési útját elmentjük a stream info objektumba a későbbi leiratoláshoz
        if result.get("recording_path") and os.path.exists(result["recording_path"]):
            stream_info = live_stream_handler.active_streams.get(stream_id)
            if stream_info:
                stream_info.recording_path = result["recording_path"]
...
async def download_video_internal(request: VideoDownloadRequest, background_tasks: BackgroundTasks, process: bool = True):
    ...
    Args:
        request: A letöltési kérés
        background_tasks: Háttérfeladatok
        process: Ha True, akkor feldolgozza a videót HLS-sé, ha False, akkor csak letölti
    """
    try:
...
async def process_audio_download(video_id: str, url: str, platform: str, 
    ...
    Args:
        ...
        url: A letöltendő videó URL-je
        platform: A platform azonosítója (auto, youtube, stb.)
        downloader: A VideoDownloader példány
        connection_id: A kapcsolat azonosítója
    """
    try:
        ...
        # Áthelyezés a Downloads mappába (közvetlenül a rendszer Downloads mappába)
...
async def process_video_download(video_id: str, url: str, platform: str, 
    ...
    try:
        ...
        if process:
            # Videó feldolgozása HLS formátumra
            ...
            # Csak letöltés esetén azonnal késznek jelöljük
...
# API végpontok az átiratoláshoz és leiratok készítéséhez
...
async def transcribe_from_url(request: dict, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Háttérfolyamat indítása az URL-ból való leiratkészítéshez
        background_tasks.add_task(
...
async def transcribe_video(video_id: str, request: dict, background_tasks: BackgroundTasks):
    """Meglévő videó átiratolása"""
    try:
        if video_id not in videos_db:
            raise HTTPException(status_code=404, detail="Video not found")
...
async def process_url_transcription(transcript_id: str, url: str, platform: str, 
    ...
    try:
        ...
        # Hibakezelés - nem írunk ki külön hiba fájlt, mert a status lekérdezéskor az "in progress" állapotban marad,
        # amit a felhasználói felület kezel időtúllépés után.
