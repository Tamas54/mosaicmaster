...
async def _transcribe_audio(audio_path: Path, identify_speakers: bool = False, fast_mode: bool = False, post_process: bool = True) -> tuple:
    if client is None:
    ...
    try:
        ...
        with open(audio_file_to_transcribe, "rb") as f:
            # Használjuk a legújabb elérhető Whisper modellt
            # Az OpenAI által kínált legújabb modell jelenleg a whisper-large-v2
            whisper_model = os.getenv("WHISPER_MODEL", "whisper-large-v2")
            try:
                transcript = await client.audio.transcriptions.create(
                    ...
                    language="hu"  # Explicit magyar nyelv megadása javítja a pontosságot
                )
            except Exception as e:
                logger.warning(f"Hiba a {whisper_model} modellel: {e}. Visszalépés az alapmodellre.")
                # Fallback az eredeti modellre hiba esetén
                transcript = await client.audio.transcriptions.create(
                    model="whisper-1",
                    file=f,
                    response_format="verbose_json",
    ...
    # Utófeldolgozás a pontosság javítására
    if post_process:
        # Gyakori hibák javítása
        corrections = {
            "vagyott": "vagyon",
            "tyúk": "sajátjuk",
            "Nemzeti Banki": "Nemzeti Bank",
            "Matolcsi": "Matolcsy",
            "cégév": "cégével",
            "pénzből pénz képzés": "pénzből pénzképzés"
        }
        ...
        # Teljes szöveg rekonstruálása a javított szegmensekből
        full_transcript_text = " ".join([s.get("text", "") for s in final_segments])
...
async def transcribe_audio(
    ...
    post_process: bool = Form(True),  # Utófeldolgozás bekapcsolva alapértelmezetten
    ...
    try:
        ...
        transcript_text, format_info = await _transcribe_audio(input_path, identify_speakers=identify_speakers, fast_mode=fast_mode, post_process=post_process)
