        ...
        if not cookies_path.exists():
            try:
                with open(cookies_path, "w") as f:
                    ...
                    # YouTube részletes cookie-k
                    f.write(f".youtube.com\tTRUE\t/\tFALSE\t{int(time.time()) + 86400}\tPREF\tid=f1=50000000&f6=8\n")
                    f.write(f".youtube.com\tTRUE\t/\tFALSE\t{int(time.time()) + 86400}\tCONSENT\tYES+cb.20220301-11-p0.en+FX+{str(random.randint(100, 999))}\n")
                    f.write(f".youtube.com\tTRUE\t/\tFALSE\t{int(time.time()) + 86400}\tYSC\t{(''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', k=11)))}\n")
                    f.write(f".youtube.com\tTRUE\t/\tFALSE\t{int(time.time()) + 86400}\tVISITOR_INFO1_LIVE\t{(''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_', k=26)))}\n")
                    # További platformok cookie-jai
                    f.write(f".twitter.com\tTRUE\t/\tFALSE\t{int(time.time()) + 86400}\tct0\t{(''.join(random.choices('0123456789abcdef', k=32)))}\n")
                    f.write(f".facebook.com\tTRUE\t/\tFALSE\t{int(time.time()) + 86400}\tdatr\t{(''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_', k=24)))}\n")
